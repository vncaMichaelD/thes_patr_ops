...
ArchivesSpace::Application.config.after_initialize do
   ...
   module SearchHelper
    ...
    def can_edit_search_result?(record)
	  ...
	  return user_can?('update_resource_record') if record['primary_type'] === "patron_reg"
	  return user_can?('update_resource_record') if record['primary_type'] === "operation"
	  return user_can?('update_resource_record') if record['primary_type'] === "patron_stat"
	  return user_can?('update_resource_record') if record['primary_type'] === "vva_event"
	  return user_can?('update_resource_record') if record['primary_type'] === "reunion"
	  return user_can?('update_resource_record') if record['primary_type'] === "conference"
